    ...
    <section id="analyzer" className="bg-mist px-6 py-16 lg:px-10 lg:py-20">
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto mb-10 max-w-xl text-center">
          <h2 className="text-[1.75rem] font-bold tracking-tight text-navy lg:text-[2rem]">Try it now</h2>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-card border border-border-base bg-white p-6 shadow-soft sm:p-8">
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-charcoal">Your writing</p>
              <button
                type="button"
                onClick={() => setText("")}
            ...
            <textarea
              ...
              className="mt-3 min-h-[300px] w-full rounded-card border border-border-base bg-white px-4 py-3 text-base leading-7 text-charcoal outline-none transition placeholder:text-[#A0AEC0] focus:border-info focus:ring-[3px] focus:ring-info/10"
            />
            ...
            <div className="mt-5 border-t border-border-base pt-5">
              ...
              >
                <span className={`flex h-5 w-5 items-center justify-center rounded-full border-2 transition ${showRubric ? "border-navy bg-navy" : "border-charcoal/30"}`}>
                  {showRubric && (
              ...
              {showRubric && (
                <div className="mt-3">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="w-full rounded-card border border-border-base bg-white px-4 py-3 text-sm leading-7 text-charcoal outline-none transition placeholder:text-[#A0AEC0] focus:border-info focus:ring-[3px] focus:ring-info/10"
                  />
                </div>
              )}
            </div>
            ...
            {error ? (
              <p className="mt-4 rounded-card border border-danger/30 bg-red-50 px-4 py-3 text-sm text-danger">
                {error}
              </p>
            ) : null}
            ...
            <div className="mt-6">
              <button
                ...
                onClick={onAnalyze}
                disabled={loading}
                className="flex items-center gap-2 rounded-lg bg-accent px-8 py-3 text-base font-bold text-navy shadow-card transition hover:bg-accent-dark hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50"
              >
                {loading ? (
                  <>
